...
def _get_response(k: str, v: str | float) -> str | float:
...
def init(
    ...
    if conf_path is None:
        conf_dir = Path(appdirs.user_config_dir("popleave"))
        conf_path = conf_dir / "config.json"

        if not conf_dir.exists():
            conf_dir.make_dirs()

    if not conf_path.exists() or force_init:
...
def get_doc_dict(       # noqa: PLR0913
    ...
    date_today = str(datetime.datetime.now(tz=datetime.timezone.utc).date())

    if start_date is None:
        start_date = _get_response(
            "Start date (YYYY-MM-DD)", date_today,
        )
...
def populate_file(
    ...
    """
    my_package = __package__ if __package__ else Path(__file__).stem
    template_file = (
...
def main(
        *, force_init: bool = False, is_toil: bool = False, **kwargs: object,
) -> bool:
    """Script for populating the leave request form."""
